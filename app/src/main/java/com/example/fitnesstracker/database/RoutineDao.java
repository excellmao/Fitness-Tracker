...
public interface RoutineDao {
    ...
    @Insert
    List<Long> insertExercises(List<Exercise> exercises); // For initial data seed

    /**
     * The magical JOIN query that connects Exercise and RoutineExercise tables.
     * This query selects the fields needed to display an exercise with its settings in a routine.
     */
    @Query("SELECT exercises.id AS exercise_id, exercises.name, exercises.category, exercises.imageUrl, exercises.level, " +
           "routine_exercises.sets, routine_exercises.reps, routine_exercises.restSeconds, routine_exercises.durationSeconds, " +
           "routine_exercises.orderIndex, routine_exercises.id AS re_id " +
